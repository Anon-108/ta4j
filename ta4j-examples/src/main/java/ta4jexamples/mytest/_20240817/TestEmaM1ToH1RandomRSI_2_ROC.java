...
public class TestEmaM1ToH1RandomRSI_2_ROC {
...
//        String filePath = "C:\\Users\\Administrator\\Desktop\\backtest\\Binance_BTCUSDT_2024-08-15_m1_1.json";
        ...
        String ExcelFileName = "D:\\Program Files\\Code\\Hengxinchuang\\ta4j\\ta4j-examples\\src\\main\\resources\\Excel\\randomEmaTest\\RandomEmaTestRSI_2_ROC_12.xlsx";
...
//        WilliamsRIndicator 威廉R指标待测试
        ROCIndicator roc = new ROCIndicator(new ClosePriceIndicator(myBackTest), 12);
...
//        String fileName = "D:\\Program Files\\Code\\Hengxinchuang\\ta4j\\ta4j-examples\\src\\main\\resources\\Excel\\20240817\\Binance_BTCUSDT_2024-08-15_m1_17.xlsx";
        ...
        for (int start = 0; start < series.getBarCount(); start++) {
            ...
            if (timeBars.isEmpty()){ //如果当前为0表示时间段开始第一个bar
                ...
                //回测
                backTest(entryLongRule,  exitLongRule,  entryShortRule,  exitShortRule, upIndicatorRule,downIndicatorRule,closeLine, openLine, endIndex, rsi, roc);
            }else {
...
//        }
    ...
    private static void backTest(Rule entryLongRule, Rule exitLongRule, Rule entryShortRule, Rule exitShortRule,
                                ...
                                , EmaV3Indicator closeLine, EmaV3Indicator openLine, int index, RSIIndicator rsi, ROCIndicator roc) {
...
//==========================ROC==============================
        double rocVal = roc.getValue(index).doubleValue();
        boolean rocBuy =false;
        boolean rocSell =false;
        if (rocVal > 1){
            rocBuy = true;
        }else if (rocVal < 1 ){
            rocSell = true;
...
//            Num emaOpen = openLine.getValue(index - 1);
        ...
        //开多：当前是多头指标并且金叉指标达成 rsi 超卖 roc 上涨趋势
        if (isEntryLong && openLong && rsiOversold && rocBuy){
            //如果当前 有了多单和空则不开
        ...
        //开空：当前是空头指标并且死叉指标达成  rsi 超买 roc 下跌趋势
        if (isEntryShort && openShort && rsiOverbuy && rocSell){
            //如果当前 有了多单和空单则不开
            if (!(Boolean)trade.get("isLong") && !(Boolean)trade.get("start") && !(Boolean)trade.get("isShort")){
